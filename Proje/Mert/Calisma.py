...
class TelefonDefter:
    ...
    def giriAl(self):
        ...
        tel_no = input("Telefon no giriniz: ")
    ...
    def Guncelleme(self,tel_id):
        try:
            adi,soyadi,tel_no = self.girisAl()
            sorgu = f"""UPDATE telefonlar SET adi = '{adi}', soyadi = '{soyadi}', tel_no = '{tel_no}' WHERE tel_id = {tel_id}"""
            self.cur.execute(sorgu)
            # commit onaylama 
            # rollback gerialma
            self.db.commit()
    ...
    def Silme(self):
        try:
            sorgu = f"""DELETE FROM telefonlar where tel_id = {tel_id}"""
            self.cur.execute(sorgu)
            # commit onaylama 
            # rollback gerialma
            self.db.commit()
